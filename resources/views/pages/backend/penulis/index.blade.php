    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                        ...
                        <div class="col-12">
                            ...
                            <div class="d-flex align-items-center justify-content-between">
                                ...
                                @can('can_create', [request()])
                                    <button type="button" class="btn btn-sm btn-outline-success waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#modalCenter" onclick="show_tambah('Tambah Penulis')">Tambah</button>
                                @endcan
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    <table id="table_penulis_rellarphp" class="table table-bordered dt-responsive  nowrap w-100 text-start">
                        <thead>
                            <tr>
                                <th  class="w-5">No</th>
                        ...
                        <tbody>
                            @foreach ($penulises as $penulis)
                                <tr>
                                    ...
                                    <td>{{$penulis->nama}}</td>
                                    <td>
                                        @if ($penulis->is_active == true)
                                            <i class='fa fa-circle text-success'></i> Aktif
                                        @elseif ($penulis->is_active == false)
                                    ...
                                    @if(auth()->user()->can('can_update', [request()]) || auth()->user()->can('can_delete', [request()]))
                                        <td>
                                            @can('can_update', [request()])
                                                <button type="button" class="btn btn-sm btn-outline-warning waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#modalCenter" onclick="show_edit('Edit Penulis',{{($penulis->id)}})" >
                                                    <i class='fa fa-pen'></i>
                                                </button>
                                            @endcan
                                            @can('can_delete', [request()])
                                                <button type='button' class='btn btn-sm btn-outline-danger waves-effect waves-light' title='Hapus' onClick="data_hapus('{{encrypt($penulis->id)}}','{{$penulis->nama}}')">
...
@section('scriptbawah')
    <script>
        $(document).ready(function() {
            var table = $("#table_penulis_rellarphp").DataTable({
                responsive: false,
                scrollX: true,
            });
        });
        //OPEN MODAL TAMBAH DATA RAK
            function show_tambah(judul){
                ...
                var act = "{{ route('penulis.modal.create')}}";
                $.ajax({
                    url: act,
        ...
        //OPEN MODAL EDIT DATA RAK
            function show_edit(judul, param){
                ...
                var act = "{{ route('penulis.modal.edit', ['param' => ':param']) }}";
        ...
        //OPEN PROSES DELETE
            function data_hapus(id, nama) {
                Swal.fire({
                    ...
                    text: `Penulis ${nama} akan dihapus!`,
                    icon: 'warning',
                    ...
                    if (result.value == true) {
                        var act = "{{ route('penulis.delete', [':param']) }}";
