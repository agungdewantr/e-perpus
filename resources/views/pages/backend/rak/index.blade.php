                        ...
                        <div class="col-12">
                            ...
                            <div class="d-flex align-items-center justify-content-between">
                                ...
                                @can('can_create', [request()])
                                    <button type="button" class="btn btn-sm btn-outline-success waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#modalCenter" onclick="show_tambah('Tambah Rak')">Tambah</button>
                                @endcan
                            </div>
                        </div>
                    </div>
                        ...
                        <tbody>
                            @foreach ($raks as $rak)
                                <tr>
                                    ...
                                    <td>{{$rak->kode}}</td>
                                    <td>
                                        @if ($rak->is_active == true)
                                            <i class='fa fa-circle text-success'></i> Aktif
                                        @elseif ($rak->is_active == false)
                                    ...
                                    @if(auth()->user()->can('can_update', [request()]) || auth()->user()->can('can_delete', [request()]))
                                        <td>
                                            @can('can_update', [request()])
                                                <button type="button" class="btn btn-sm btn-outline-warning waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#modalCenter" onclick="show_edit('Edit Rak',{{($rak->id)}})" >
                                                    <i class='fa fa-pen'></i>
                                                </button>
                                            @endcan
                                            @can('can_delete', [request()])
                                                <button type='button' class='btn btn-sm btn-outline-danger waves-effect waves-light' title='Hapus' onClick="data_hapus('{{encrypt($rak->id)}}','{{$rak->kode}}')">
    ...
    <script>
        ...
        //OPEN MODAL TAMBAH DATA RAK
            function show_tambah(judul){
                ...
                var act = "{{ route('rak.modal.create')}}";
                $.ajax({
                    url: act,
        ...
        //OPEN MODAL EDIT DATA RAK
            function show_edit(judul, param){
                ...
                var act = "{{ route('rak.modal.edit', ['param' => ':param']) }}";
        ...
        //OPEN PROSES DELETE
            function data_hapus(id, kode) {
                Swal.fire({
                    title: 'Apakah anda yakin?',
                    text: `Kode ${kode} akan dihapus!`,
                    icon: 'warning',
                    ...
                    if (result.value == true) {
                        var act = "{{ route('rak.delete', [':param']) }}";
