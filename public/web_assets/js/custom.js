...
var Bookland = function(){
	...
	var handleShopCart = function(){
        ...
        $('.cart-btn2').unbind().on('click',function(){
			$(".cart-list2").slideToggle('slow');
		})
	...
	var cartButton = function(){
        ...
        $(document).on('click', function (event) {
            if (!$(event.target).closest('.cart-list, .cart-btn').length) {
                $(".cart-list").slideUp('slow');
            }
            // Menutup .cart-list2 saat mengklik di luar .cart-list2
            if (!$(event.target).closest('.cart-list2, .cart-btn2').length) {
                $(".cart-list2").slideUp('slow');
            }
        });
